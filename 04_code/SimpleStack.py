...
class Stack(object):
   ...
   def push(self, item):               # Insert item at top of stack
      if self.isFull():
         raise OverflowError("La pila esta llena no puede ingresar mas elementos")
      self.__top += 1                  # Advance the pointer
      self.__stackList[self.__top] = item  # Store item
        
   def pop(self):
      if self.isEmpty():
         raise Exception("La pila no contiene elementos para poder vaciar")                      # Remove top item from stack
